...
# ── Load Model (cached) ──────────────────────────────────────
import os
BASE_DIR = r"D:\同步文件夹\Radiomics\paper2"

@st.cache_resource(show_spinner="Loading model, please wait…")
def load_model():
    h2o.init(verbose=False)
    return h2o.load_model(os.path.join(BASE_DIR, "models", "GLM_1_AutoML_2_20260408_210958"))
...
if predict_btn:
    ...
    with st.spinner("Computing SHAP values…"):
        try:
            ...
            bg_df  = pd.read_csv(os.path.join(BASE_DIR, "models", "background.csv"))
